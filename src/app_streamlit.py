...
# Inicializar session_state para prefill
...
if transaction_type == "MULTIPLAS":
    ...
    has_existing_transaction = st.radio(
        ...
        key="has_prefill_radio"
    )

    # Salvar escolha
    st.session_state.has_prefill = has_existing_transaction

    if has_existing_transaction == "Sim":
        st.info("ℹ️ Cole o array `\"transactions\"` do JSON. O sistema extrairá os dados para pré-preencher o formulário.")

        existing_transactions_str = st.text_area(
            "Cole aqui o JSON das transações existentes:",
            height=200,
            placeholder="""[
  {
    "amount": 284050,
    ...
    "payment_fields": {
      ...
      ...
            ...
            help="Cole apenas o array de transactions, ou o JSON completo (será extraído automaticamente)",
            key="prefill_json_input"
        )

        if existing_transactions_str.strip():
            try:
                # Tentar fazer parse do JSON
                parsed = json.loads(existing_transactions_str.strip())

                # Se for um objeto com campo "transactions", extrair
                ...
                # Se já for um array, usar diretamente
                    ...
                    st.warning("⚠️ Formato não reconhecido. Cole um array de transações ou JSON completo.")

                if prefill_data:
                    # Armazenar em session_state
                    st.session_state.prefill_data = prefill_data
                    st.success(f"✅ {len(prefill_data)} transação(ões) detectada(s) e pronta(s) para pré-preenchimento!")
...
if st.session_state.prefill_data:
    prefill_data = st.session_state.prefill_data
